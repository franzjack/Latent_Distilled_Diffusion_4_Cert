...
parser.add_argument("--latent_dim", type=int, default=75, help="dimensionality of the latent space")
parser.add_argument("--n_critic", type=int, default=5, help="number of training steps for discriminator per iter")
parser.add_argument("--traj_len", type=int, default=30, help="number of steps")
...
parser.add_argument("--model_name", type=str, default="CROSS", help="name of the model")
...
parser.add_argument("--loading_id", type=str, default="277", help="id of the model to load")
...
trainset_fn = f"../../data/{model_name}/{model_name}_map_data_train.pickle"
testset_fn = f"../../data/{model_name}/{model_name}_map_data_test.pickle"
#validset_fn = f"../../data/{model_name}/{model_name}_map_data_test.pickle"
...
def compute_gradient_penalty(C, real_samples, fake_samples, lab, goals):
    ...
    c_interpolates = C(interpolates, lab,goals)
...
if opt.training_flag:
    ...
    for epoch in range(opt.n_epochs):
        ...
        for i in range(bat_per_epo):
            trajs_np, conds_np, goal_np = ds.generate_mini_batches(opt.batch_size)
            # Configure input
            real_trajs = Variable(Tensor(trajs_np))
            conds = Variable(Tensor(conds_np))
            goals = Variable(Tensor(goal_np))
            # ---------------------
            #  Train Critic
            # ---------------------
            ...
            fake_trajs = generator(z, conds, goals)
            
            # Real images
            real_validity = critic(real_trajs, conds,goals)
            # Fake images
            fake_validity = critic(fake_trajs, conds,goals)
            # Gradient penalty
            gradient_penalty = compute_gradient_penalty(critic, real_trajs.data, fake_trajs.data, conds.data,goals.data)
            # Adversarial loss
            ...
            if i % opt.n_critic == 0:
                ...
                gen_goals = Variable(Tensor(generate_random_conditions()))

                #print("input shape is",gen_conds.shape, "\n")
                #print("conditioning shape is ",z.shape, "\n")

                # Generate a batch of images
                gen_trajs = generator(z, gen_conds,gen_goals)

                # Loss measures generator's ability to fool the discriminator
                # Train on fake images
                fake_validity = critic(gen_trajs, gen_conds,gen_goals)
...
if TEST_TRAJ_FLAG:
    ...
    for iii in range(ds.n_points_test):
        ...
        for jjj in range(n_gen_trajs):
            ...
            temp_out = generator(Variable(Tensor(z_noise)), Variable(Tensor([ds.Y_test_transp[iii,jjj]])),Variable(Tensor([ds.Z_test_transp[iii,jjj]])))
...
if TEST_PLOT_FLAG:
    ...
    for kkk in range(0, ds.n_points_test, int(ds.n_points_test/50)):
        ...
        pgoal = np.array([(ds.HMIN+(ds.Z_test_transp[kkk,it].T+1)*(ds.HMAX-ds.HMIN)/2).T for it in range(ds.n_test_traj_per_point)])
        ...
        ax.scatter([pgoal[:,0,-1]], [pgoal[:,1,-1]],c='r')
        ...
        fig.suptitle('cwgan-MAZE',fontsize=40)
        plt.tight_layout()
