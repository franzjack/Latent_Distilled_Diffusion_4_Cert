...
class Dataset(Dataset):
	def __init__(self, model_name, target_dim, eval_length, missing_ratio=0.0, seed=0, datafolder='', idx='', scaling_flag=False, extra_info = '', train_ds = None, path= ''):
		...
		use_index_list = None
		if scaling_flag:
			newpath = (path+f"/data/"+datafolder+"/"+model_name+"_norm_"+idx+f"_missing{missing_ratio}_gtmask{extra_info}.pickle")
		else:
			newpath = (path+f"/data/"+datafolder+"/"+model_name+"_"+idx+f"_missing{missing_ratio}_gtmask{extra_info}.pickle")
		if not os.path.isfile(newpath):  # if datasetfile is none, create

			if idx == 'test_fixed':
				observed_values, mask = self.build_mask(path+f'/data/'+datafolder+'/'+datafolder+'_data_test_fixed_froms.pickle', missing_ratio, use_index_list)
			else:	
				observed_values, mask = self.build_mask(path+f'/data/'+datafolder+'/'+datafolder+'_map_data_'+idx+'.pickle', missing_ratio, use_index_list)
