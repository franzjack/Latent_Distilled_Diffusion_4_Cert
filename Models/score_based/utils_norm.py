...
def new_evaluate_implicit(model, test_loader, nsample=100, scaler=1, mean_scaler=0, foldername="", ds_id = 'test'):
    ...
    with torch.no_grad():
        ...
        with tqdm(test_loader, mininterval=5.0, maxinterval=50.0) as it:
            ...
            for batch_no, test_batch in enumerate(it, start=1):
                #print(test_batch['timepoints'].shape)
                output = model.evaluate_implicit(test_batch, nsample)
...
def plot_rescaled_crossroads(opt, foldername, dataloader, nsample, idx = 'test'):
    ...
    #plt.legend()
    plt.tight_layout()
    fig.savefig(foldername+f'CSDI_{opt.model_name}_crossroad_{idx}_{i}.png')
    plt.close()


def plot_rescaled_crossroads2(opt, foldername, dataloader, nsample, idx = 'test'):
    plt.rcParams.update({'font.size': 25})
    ...
    for dataind in range(M):
        ...
        #plt.legend()
        plt.tight_layout()
        fig.savefig(foldername+f'CSDI_{opt.model_name}_crossroad_{idx}_{dataind}.png')
        plt.close()
