...
# CSDI base class for diffusion models, using the clean data (x) as target.
...
class CSDI_base(nn.Module):
    def __init__(self, target_dim, config, device, teacher_model = None, alphas=None):
        ...
        if config_diff["schedule"] == "quad":
            ...
            # schedule for student model for distillation, copies teacher model schedule
            # and rescale it to half the steps
            ...
            # custom schedule, user-defined alphas (used for steps of distillation other than the first one)
            # alphas should be a tensor of shape (num_steps,)
            if alphas is None:
                raise ValueError("Reference model must be provided for custom schedule.")
            else:
                self.alpha = alphas
                self.alpha = self.alpha.to(self.device)
                self.alpha_hat = torch.empty_like(self.alpha)
