...
class diff_CSDI(nn.Module):
    def __init__(self, config, inputdim, traj_len):
        ...
        self.input_projection = Conv_block(self.K, self.channels, 1)
        self.output_projection1 = Conv_block(self.K, self.channels, 3)
        self.output_projection2 = Conv_block(self.channels, 2* self.K, 3)
...
class ResidualBlock(nn.Module):
    def __init__(self, channels, diffusion_embedding_dim,K,L):
        ...
        #self.mid_time_projection = Conv_block(L, channels, 3)
        #self.output_time_projection = Conv_block(channels, L, 3)

        #self.time_layer = get_torch_trans(heads=nheads, layers=1, channels=channels)
        #self.feature_layer = get_torch_trans(heads=nheads, layers=1, channels=channels)
        #self.time_layer = Conv1D_embedding(in_channels=channels,out_channels=channels, kernel_size=3)
        #self.feature_layer = Conv1D_embedding(in_channels=channels,out_channels=channels, kernel_size=3)
