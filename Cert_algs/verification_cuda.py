...
def grad_ascent(qmodel, z0, lr= 0.01, tol = 10^(-6)):
...
def traj_verifier(cmodel, zopt, epsilon, latent_dim = 50, maxes = None):
  ...
  bounded_model_bstl = BoundedModule(cmodel, torch.zeros_like(zopt), bound_opts={"conv_mode": "tensor"}, verbose=True)
...
def verifier_increment(bmodel, qmodel, M=1, epsilon = 0.0005, delta_eps = 0.0005, latent_shape = Tensor(np.empty((1,50)))):
  ...
  for i in range(M):
    ...
    while(rob<0):
      ...
      rob = qmodel(Zstar)
      print('Iteration of gradient ascent number: ', it)
      it+=1
    ...
    bounded_model_bstl = BoundedModule(bmodel, Zstar, bound_opts={"conv_mode": "tensor",'sparse_intermediate_bounds': False,
    ...
    while(loc_flag == 1):
      ...
      if loc_flag == 1:
        ver_flag = 1
        last_ball = (Zstar, epsilon, lb_bstl, ub_bstl)
        epsilon += delta_eps
...
def verifier_increment2(bmodel, qmodel, M=1, epsilon = 0.001, delta_eps = 0.0005, latent_shape = Tensor(np.empty((1,50)))):
...
def verifier_heterog_increment(bmodel, qmodel, weight = 0.1, M=1, epsilon = 0.0005, delta_eps = 0.0005, latent_shape = Tensor(np.empty((1,50))), model_id = ''):
  ...
  for i in range(M):
    ...
    bounded_model_bstl = BoundedModule(bmodel, zstar_i, bound_opts={"conv_mode": "tensor",'sparse_intermediate_bounds': False,
            'sparse_conv_intermediate_bounds': False,'sparse_intermediate_bounds_with_ibp': False},verbose=True)
    bounded_model_bstl.eval()
    ...
    while(loc_flag == 1):
      ...
      print(f'eps = {eps_vec}')
      ptb_vec = PerturbationLpNorm(norm = norm, eps = eps_vec)
      bounded_z = BoundedTensor(zstar_i, ptb_vec)
    ...
    if ver_flag == 1:
      ...
      list_path = os.path.join('results','GAN',model_id+'_HET_N={len(B_list)}.pkl')
...
def truncated_samples_tensor(ball_i, latent_shape, latent_dim, nsamples):
    LB = (ball_i[0]-ball_i[1]).detach().cpu().numpy()
    UB = (ball_i[0]+ball_i[1]).detach().cpu().numpy()
    Z_single = torch.empty_like(latent_shape).unsqueeze(0)
    Z = Z.repeat(nsamples)
...
def empirical_satisfaction(ball_list, qmodel, latent_dim, nsamples = 100, model_name = '',cert_type='HOM'):
  ...
  Z = torch.empty((nsamples,latent_dim), device=device)
  c=0
  ...
  for i in range(M):
    ...
    Zi = truncated_samples(ball_i, latent_dim, nsample_i)
  ...
  plt.tight_layout()
  figpath = os.path.join('results','GAN',model_name, 'EXP','diff_'+model_name+'_certified_samples_'+str(nsamples)+'.png')
  fig.savefig(figpath)
  plt.close()
