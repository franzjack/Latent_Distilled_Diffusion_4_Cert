...
def get_trajs(model, cond, latent_samples, mins, maxs):
	...
	scaled_trajs = model(latent_samples, conds)

	return mins.unsqueeze(1).unsqueeze(0)+((scaled_trajs.permute(0,2,1)+1)*(maxs-mins)/2).permute(0,2,1)
...
def vanilla_samples(model, cond, nsamples, latent_dim, mins, maxs):
	...
	scaled_trajs = model(noises, conds)
...
def diff_vanilla_samples(model, cond, nsamples, latent_shape, mins, maxs):
	...
	noises = torch.randn_like(latent_shape)
