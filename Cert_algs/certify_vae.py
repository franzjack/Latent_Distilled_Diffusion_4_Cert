...
ae_foldername = ""
ae_foldername = "./save/fVAE/fVAE_201/"
...
print("decoded shape is", vae.decoder(xx).shape)
print("shape to be decoded is", xx[0].shape)
print("decoded shape is", vae.decoder(xx[0]).shape)
print("xx device is",xx.device)


Zstar = torch.randn(1, ae_latent_dims)
...
print('Original VAE decoder prediction:', gen_mod(Zstar))



output_vae = verifier_vanilla(gen_mod, M=M_MAX, eps_start = EPS, Zstar = Zstar, model_id = "MNIST_VAE")

#output = verifier_vanilla(model, M=M_MAX, eps_start = EPS, Zstar = Zstar, model_id = "MNIST")
